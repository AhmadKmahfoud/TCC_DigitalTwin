...
INT main(VOID)
{
    ...
    if (dst_ip_address[0] == 0)
        strcpy(dst_ip_address, "192.168.3.39");
        ...
        while (retry)
        {
            ...
            { // If it receives by the time-out
                ...
                if (MXTrecv.SendIOType == MXT_IO_IN)
                    ...
                    sprintf(str, "------");
                int DispType;
                void* DispData = nullptr; //Henrique modificacao depois do "="
                switch (disp_data)
                {
                case 0:
                ...
                case MXT_TYP_FB_POSE:
                    ...
                    if (disp)
                    {
                        ...
                        sprintf(buf, "Receive (%ld): TCount=%dType(POSE)=%d\n %7.2f,%7.2f,%7.2f,%7.2f,%7.2f,%7.2f, %04x,%04x (%s)", MXTrecv.CCount, MXTrecv.TCount, DispType, p->w.x, p->w.y, p->w.z, p->w.a, p->w.b, p->w.c, p->sflg1, p->sflg2, str);
                        cout << buf << endl;
                    }
                    break;
                case MXT_TYP_PULSE:
